(function (factory, global) {
	...
	Chosen.prototype.buildResults = function (items) {
		...
		if (!items || items.length === 0) {
			...
			if (this.query) {
				...
				$queryElement.innerHTML = this.query;

				$noResults.innerHTML = this.options.noResultsText || 'No results match ';

				$noResults.appendChild($queryElement);

			} else {

				$noResults.innerHTML = this.options.emptyListText || 'No items in list';
			}
		...
		return items.map(function (item, index) {
			...
			if (typeof options.renderResult === 'function') {
				$result.innerHTML = options.renderResult(item, query, selectedValue);
			} else {
	...
	Chosen.prototype.setSelectedValue = function (value) {
		...
		if (typeof this.options.renderSelectedValue === 'function') {
			this.$selectedValueContainer.innerHTML = this.options.renderSelectedValue(selectedItem, this.query);
		} else {
			this.$selectedValueContainer.innerHTML = selectedItemText;
		}
